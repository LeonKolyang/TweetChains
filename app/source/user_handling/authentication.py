...
def get_access_token(oauth_token, oauth_token_secret, oauth_verifier) -> dict:
    ...
    try:
        ...
        logger.error("There may have been an issue with the consumer secrets or user secrets you entered.")
        logger.debug(e)
        oauth_tokens = {}
    return oauth_tokens


def load_oauth_session(access_token, access_token_secret):
        # Make the request
    oauth = OAuth1Session(
        consumer_key,
        client_secret=consumer_secret,
